...
class ProjectAbridgement {
    ...
    public render(container: HTMLElement): void {
        ...
        this.project.skills.forEach(v => {
            skillList += `<img title=${v} style="background: #5288F1; width: 36px; height: 36px;">`;
        });
        
        container.innerHTML += `
            <div style="padding: 20px; width: calc((100% - 40px) / 3); background: white; border-radius: 16px; border: 1px solid #DADADA; display: flex; flex-direction: column; gap: 20px;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span class="recruitPeriod">${this.project.recruitStartDate} ~ ${this.project.recruitEndDate}</span>
                ...
                <div style="display: flex; justify-content: space-between; flex-wrap: wrap;">
                    <div style="display: flex; gap: 20px;">
                        <span>현재 인원</span>
                        <span class="currentMember">${this.project.currentMember}</span>
                    </div>
                    <div style="display: flex; gap: 20px;">
                        <span>추가 모집</span>
                        <span class="additionalRecruitMember">${this.project.additionalRecruitMember}</span>
                    </div>
                </div>
                <div>
                    <span>사용 기술 스택</span>
                ...
                <div style="display: flex; justify-content: flex-end; flex-wrap: wrap; gap: 20px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <span>${this.project.watchCount}</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <svg width="24" height="24">
                        ...
                        <span>${this.project.goodCount}</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <svg width="24" height="24">
                        ...
                        <span>${this.project.mentionCount}</span>
                    </div>
                </div>
            </div>`;
    }
